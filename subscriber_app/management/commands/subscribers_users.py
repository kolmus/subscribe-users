    ...
    def handle(self, *args, **options):
            ...
            if subscriber.email in user_emails:
                ...
                if not users.filter(phone=client.phone).exclude(email=client.email).exists():       # and not(user.phone = client.phone and user.email != client.email)
                    ...
                    if client.phone not in clients_ext_phones:
                        clients_to_migrate.append(Users(
                            phone=client.phone,
                            email=client.email,
                ...
                subscribers_to_migrate.append(Users(
        ...
        for smssubscriber in sms_subscribers:
            if smssubscriber.phone in user_phones:
                ...
                subscribers_to_migrate.append(Users(
                    phone=smssubscriber.phone,
                    gdpr_consent=smssubscriber.gdpr_consent
                ))
        
        # Save in database
        if clients_to_migrate:
            new_cl_users = Users.objects.bulk_create(clients_to_migrate)
        if subscribers_to_migrate:
            new_sub_users = Users.objects.bulk_create(subscribers_to_migrate)
        if sms_subscribers_to_migrate:
            new_subsms_users = Users.objects.bulk_create(sms_subscribers_to_migrate)
